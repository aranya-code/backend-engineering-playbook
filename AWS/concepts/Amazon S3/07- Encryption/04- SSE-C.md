...
SSE-C requires the caller to present the customer key for every read, write, copy, and metadata operation. Losing that key makes the object unrecoverable, so it is rarely a good modern backend default.
